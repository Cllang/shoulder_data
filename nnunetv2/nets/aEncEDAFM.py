...
class MFDAFM(nn.Module):
    ...
    def forward(self, x):
        ...
        high_freq = x - F.interpolate(low_freq, size=x.size()[2:], mode='nearest')

        # 低频特征通过注意力增强
        low_freq = F.interpolate(self.attention(low_freq), size=x.size()[2:], mode='nearest')
    ...
    def __init__(self,
        ...
        for s in range(n_stages):
            stage = nn.Sequential(
                ...
                *[MFDAFM(features_per_stage[s]) for _ in range(n_blocks_per_stage[s] - 1)]
                # *[
                #     BasicBlockD(
                #         conv_op=conv_op,
...
def get_aEncEDAFM_from_plans(
    ...
    segmentation_network_class_name = 'aEncEDAFM'
    network_class = aEncEDAFM
    kwargs = {
        'aEncEDAFM': {
...
if __name__ == '__main__':
    ...
    model = get_aEncEDAFM_from_plans(
        plans_manager,
        dataset_json,
