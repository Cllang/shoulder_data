...
class DualBranchFusion(nn.Module):
    def __init__(self, channels):
        ...
        self.conv_standard = nn.Conv3d(channels, channels, kernel_size=3, padding=1, bias=False)
        self.conv_dilated = nn.Conv3d(channels, channels, kernel_size=3, padding=2, dilation=2, bias=False)
...
class MFDAFM(nn.Module):
    def __init__(self, channels):
        ...
        self.down = nn.AvgPool3d(kernel_size=2)
        self.attention = SimpleAttention(channels)
        self.dual_branch_fusion = DualBranchFusion(channels)
        self.conv_out = nn.Conv3d(channels, channels, kernel_size=3, padding=1, bias=False)

    def forward(self, x):
        # 多频分解
        low_freq = self.down(x)
        high_freq = x - F.interpolate(low_freq, size=x.size()[2:], mode='trilinear', align_corners=True)

        # 低频特征通过注意力增强
        low_freq = F.interpolate(self.attention(low_freq), size=x.size()[2:], mode='trilinear', align_corners=True)
...
def get_EncEDAFM_from_plans(
    ...
    segmentation_network_class_name = 'EncEDAFM'
    network_class = EncEDAFM
    kwargs = {
        'EncEDAFM': {
