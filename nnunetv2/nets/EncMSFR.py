...
class FeatureRefinementModule(nn.Module):
    def __init__(self, in_dim=128, out_dim=128):
        ...
        # 使用不同尺度的卷积核
        self.lconv = nn.Conv3d(in_dim, in_dim, kernel_size=(3, 1, 1), stride=1, padding=(1,0,0), groups=in_dim)
        self.mconv = nn.Conv3d(in_dim, in_dim, kernel_size=(1,3,1), stride=1, padding=(0,1,0), groups=in_dim)
        self.hconv = nn.Conv3d(in_dim, in_dim, kernel_size=(1,1,3), stride=1, padding=(0,0,1), groups=in_dim)
        ...
        # 下采样操作
        self.down = nn.Conv3d(in_dim, in_dim, kernel_size=3, stride=2, padding=1, groups=in_dim)
        # 1x1 卷积用于特征融合
    ...
    def forward(self, x):
        ...
        # 下采样
        dx = self.down(x)
        udx = F.interpolate(dx, scale_factor=2, mode='nearest')

        # 使用不同尺度的卷积核进行处理
        lx = self.norm1(self.lconv(self.act(x * udx)))
        mx = self.norm2(self.mconv(self.act(x * udx)))
        hx = self.norm3(self.hconv(self.act(x * udx)))
...
class AFE(nn.Module):
    def __init__(self, dim, kernel_size=3):
        ...
        self.norm3 = nn.InstanceNorm3d(dim // 2, eps=1e-6)
...
def get_EncMSFR_from_plans(
    ...
    segmentation_network_class_name = 'EncMSFR'
    network_class = EncMSFR
    kwargs = {
        'EncMSFR': {
