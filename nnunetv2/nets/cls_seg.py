...
class ResidualMambaEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for s in range(len(self.stages)):
            ...
            ret.append(x)
        # ret[-1].shape : torch.Size([1, 320, 6, 6, 6])
...
class UMambaEnc(nn.Module):
    ...
    def forward(self, x):
        skips, cls_head = self.encoder(x)
        return self.decoder(skips), cls_head
...
class NiftiDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        cop = tio.CropOrPad((12, 384, 384))
        zs = tio.ZNormalization()
        ...
        label = int(self.label_files[idx][-8])
        

        # img = np.expand_dims(img, axis=0)
        # mask = np.expand_dims(mask, axis=0)

        return torch.tensor(img.numpy()), torch.tensor(mask.numpy()), torch.tensor(label)
        # return torch.tensor(img.numpy()), torch.tensor(mask.numpy())
...
def compute_loss(seg_output, cls_output, seg_target, cls_target, seg_weight=0.5, cls_weight=0.5):
    loss_seg = dice_loss(seg_output, seg_target)
    cls_loss_fn = nn.CrossEntropyLoss()
    loss_cls = cls_loss_fn(cls_output, cls_target)
...
def train():
    ...
    dataset = NiftiDataset("/home/fenge/Dataset703_cuff934/imagesTr", "/home/fenge/Dataset703_cuff934/labelsTr")
    loader = DataLoader(dataset, batch_size=2, shuffle=True)

    optimizer = optim.Adam(model.parameters(), lr=1e-4)

    best_loss = float('inf')
    save_dir = '/home/fenge/checkpoints'
    ...
    for epoch in range(1, 21):
        ...
        # for img, mask in tqdm(loader):
            ...
            print(seg_output.shape, mask.shape)
            # seg_output = model(img)
            
            loss, loss_seg, loss_cls = compute_loss(seg_output, cls_output, mask, label)
            # loss = dice_loss(seg_output, mask)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        epoch_loss_seg = total_loss_seg / len(loader)
        epoch_loss_cls = total_loss_cls / len(loader)
        epoch_loss = total_loss / len(loader)
        epoch_dice = running_dice / len(loader)
        print(f"Epoch {epoch}, Loss: {epoch_loss:.4f}, Dice Score: {epoch_dice:.4f}, Loss_seg: {epoch_loss_seg:.4f}, Loss_cls: {epoch_loss_cls:.4f}")
        ...
        if epoch_loss < best_loss:
            ...
            print(f"✅ Saved best model (loss={best_loss:.4f}) to {best_path}")
