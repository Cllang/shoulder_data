...
class UpsampleLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        print(self.pool_op_kernel_size)
        x = self.conv(x)
        return x
...
class ResidualMambaEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for s in range(len(self.stages)):
            ...
            issa = InterSliceSelfAttention(in_dim=in_channel, q_k_dim=16, patch_ini=input_resolution, axis='D').cuda()
            x = issa(x, x)
            ret.append(x)
...
def get_myNet_from_plans(
    ...
    segmentation_network_class_name = 'EncISSA'
    network_class = EncISSA
    kwargs = {
        'EncISSA': {
...
if __name__ == '__main__':
    ...
    plans = load_json(
        '/data/item/U-Mamba/data/nnUNet_results/Dataset703_cuff934/nnUNetTrainerUMambaEnc__nnUNetPlans__3d_fullres/plans.json')
    plans_manager = PlansManager(plans)
    configuration_manager = plans_manager.get_configuration('3d_fullres')
    dataset_json = load_json(
        '/data/item/U-Mamba/data/nnUNet_results/Dataset703_cuff934/nnUNetTrainerUMambaEnc__nnUNetPlans__3d_fullres/dataset.json')
    num_input_channels = determine_num_input_channels(plans_manager, configuration_manager, dataset_json)
    model = get_myNet_from_plans(
        plans_manager,
        dataset_json,
