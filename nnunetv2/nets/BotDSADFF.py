...
class DirectionalSliceAttention(nn.Module):
    def __init__(self, in_dim, q_k_dim, patch_ini):
        """
        初始化方法，定义了卷积层和位置嵌入。
        Parameters:
        in_dim : int  # 输入张量的通道数
        ...
        """
        super(DirectionalSliceAttention, self).__init__()
        self.in_dim = in_dim
        ...
        self.ca = ChannelAttention(in_planes=in_dim)
...
class UNetResEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for i,s in enumerate(self.stages):
            ...
            if i < 4:
                input_resolution, in_channel = tuple(int(i) for i in x.shape[2:]), int(x.shape[1])
                dsa = DirectionalSliceAttention(in_dim=in_channel, q_k_dim=16, patch_ini=input_resolution).cuda()
                x = dsa(x, x)
            ret.append(x)
...
def get_BotDSADFF_from_plans(
    ...
    segmentation_network_class_name = 'BotDSADFF'
    network_class = BotDSADFF
    kwargs = {
        'BotDSADFF': {
            'conv_bias': True,
...
if __name__ == '__main__':
    ...
    plans = load_json('/data/item/U-Mamba/data/nnUNet_results/Dataset703_cuff934/nnUNetTrainerUMambaBot__nnUNetPlans__3d_fullres/plans.json')
    plans_manager = PlansManager(plans)
    configuration_manager = plans_manager.get_configuration('3d_fullres')
    dataset_json = load_json('/data/item/U-Mamba/data/nnUNet_results/Dataset703_cuff934/nnUNetTrainerUMambaBot__nnUNetPlans__3d_fullres/dataset.json')
    num_input_channels = determine_num_input_channels(plans_manager, configuration_manager, dataset_json)
    model = get_BotDSADFF_from_plans(
        plans_manager,
        dataset_json,
