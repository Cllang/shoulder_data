...
class MSGDC(nn.Module):
    def __init__(self, in_chn, dilation1=1, dilation2=3, dilation3=5, kernel_size=3, stride=1, padding='same'):
        ...
        self.cov1 = nn.Conv3d(in_chn, in_chn, kernel_size, stride, padding, dilation=dilation1, bias=True)
        self.cov2 = nn.Conv3d(in_chn, in_chn, kernel_size, stride, padding, dilation=dilation2, bias=True)
        self.cov3 = nn.Conv3d(in_chn, in_chn, kernel_size, stride, padding, dilation=dilation3, bias=True)
    ...
    def forward(self, x):
        B, C, H, W, D = x.shape
        x = self.move(x)
        x1 = self.cov1(x).permute(0, 2, 3, 4, 1).flatten(1, 3)
        x1 = self.act1(x1)
        x2 = self.cov2(x).permute(0, 2, 3, 4, 1).flatten(1, 3)
        x2 = self.act2(x2)
        x3 = self.cov3(x).permute(0, 2, 3, 4, 1).flatten(1, 3)
        x3 = self.act3(x3)
        x = self.norm(x1 + x2 + x3)
        return x.permute(0, 2, 1).view(-1, C, H, W, D).contiguous()
    ...
    def __init__(self,
        ...
        for s in range(n_stages):
            stage = nn.Sequential(
                ...
                *[MSGDC(in_chn=features_per_stage[s]) for _ in range(n_blocks_per_stage[0] - 1)]
                # *[
                #     BasicBlockD(
                #         conv_op=conv_op,
...
def get_EncMSGDC_from_plans(
    ...
    segmentation_network_class_name = 'EncMSGDC'
    network_class = EncMSGDC
    kwargs = {
        'EncMSGDC': {
